...
1. Открыть Zabbix Web UI: http://5.159.101.3:8082

**⚠️ Важно:** При первом запуске Zabbix может показывать ошибку "Database error - Unable to select configuration". Это нормально, так как база данных еще инициализируется. Подождите 3-5 минут и обновите страницу.
...
4. Настроить хост для мониторинга Nextcloud:
   - Host name: `nextcloud`
   - IP address: `nextcloud` (имя контейнера, не числовой IP!)
   - Port: `80`
   - Применить шаблон "Test ping template"
...
1. Перейти в Connections → Plugins
2. Найти и установить плагин "Zabbix"

**⚠️ Если видите "You do not have permission to install this plugin":**
```bash
...
3. Указать URL: `http://loki:3100`
   - **⚠️ ВАЖНО:** Используйте именно `loki` (имя контейнера), а НЕ `localhost`!
...
2. Указать URL Zabbix API: `http://zabbix-front:8080/api_jsonrpc.php`
   - **⚠️ ВАЖНО:** Используйте именно `zabbix-front` (имя контейнера), а НЕ `localhost`!
